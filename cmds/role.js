...
async function addRole(member, role) {
  ...
  if (member.roles.cache.has(role.id))
    return { content: `You're already part of ${role.name}.`, ephemeral: true };
...
async function removeRole(member, role) {
  ...
  if (!member.roles.cache.has(role.id))
    return { content: `You are not part of ${role.name}.`, ephemeral: true };
...
async function assignRole(target, role) {
  ...
  if (!leadershipRoles.some((r) => role.name === r))
    return { content: "Please pass in a leadership role.", ephemeral: true };
...
async function unassignRole(target, role) {
  ...
  if (!leadershipRoles.some((r) => role.name === r))
    return { content: "Please pass in a leadership role.", ephemeral: true };
  if (!target.roles.cache.has(role.id))
    return { content: `${targetName} is not assigned the ${role.name} role.` };
