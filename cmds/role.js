...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('command').setDescription('Do you want to add or remove a role?').setRequired(true)
            .addChoices([
                ['Add', 'add'],
                ['Remove', 'remove'],
        ...
        .addUserOption(option =>
            option.setName('target').setDescription('Target member to (un)assign a role to/from (Leadership only)').setRequired(false)),
    aliases: ['roles'],
    ...
    execute: async (bot, message, args) => {
        ...
        if (args.length >= 1) {
            ...
            if (cmd === 'assign') {
                if (!await leadershipCheck(message)) return 'You are not allowed to do that.'
                ...
                try {
                    ...
                    if (memberAssign.roles.cache.has(roleAssign.id)) return `${name} is already part of ${roleAssign.name}.`
            ...
            if (cmd === 'unassign') {
                ...
                try {
                    ...
                    if (!memberAssign.roles.cache.has(roleUnassign.id)) return `${name} is not part of ${roleUnassign.name}.`
            ...
            if (cmd === 'add') {
                ...
                if (leadershipRoles.some(r => role.name === r)) return `That's illegal!`
                if (member.roles.cache.has(role.id)) return `You're already part of ${role.name}.`
                member.roles.add(role);
                return `You've been added to ${role.name}.`
            }
            if (cmd === 'remove') {
                const member = message.member;
                if (leadershipRoles.some(r => role.name === r)) return 'You can only be manually removed from that role by a leadership member.'
                if (!member.roles.cache.has(role.id)) return `You are not part of ${role.name}.`
    ...
    interact: async interaction => {
        ...
        try {
            ...
            if (isLeader && command === 'assign') {
                if (target === null) return interaction.reply({ content: 'Please include a target user.', ephemeral: true })
...
async function addRole(member, role) {
    if (barredRoles.some(r => role.name === r)) return { content: `That's illegal!`, ephemeral: true }
    if (member.roles.cache.has(role.id)) return { content: `You're already part of ${role.name}.`, ephemeral: true }
    await member.roles.add(role)
    return { content: `You've been added to ${role.name}.`, ephemeral: true }
}

async function removeRole(member, role) {
    if (barredRoles.some(r => role.name === r)) return { content: 'You can only be manually removed from that role by a leadership member.', ephemeral: true }
    if (!member.roles.cache.has(role.id)) return { content: `You are not part of ${role.name}.`, ephemeral: true }
    await member.roles.remove(role);
    return { content: `You've been removed from ${role.name}.`, ephemeral: true }
}

async function assignRole(target, role) {
    const targetName = (target.nickname) ? target.nickname : target.user.username
    if (!leadershipRoles.some(r => role.name === r)) return { content: 'Please pass in a leadership role.', ephemeral: true }
    if (target.roles.cache.has(role.id)) return { content: `${targetName} is already assigned the ${role.name} role.` }
    await target.roles.add(role)
    return { content: `<@!${target.id}>, you have been assigned the ${role.name} role.` }
}

async function unassignRole(target, role) {
    const targetName = (target.nickname) ? target.nickname : target.user.username
    if (!leadershipRoles.some(r => role.name === r)) return { content: 'Please pass in a leadership role.', ephemeral: true }
    if (!target.roles.cache.has(role.id)) return { content: `${targetName} is not assigned the ${role.name} role.` }
    await target.roles.remove(role)
    return { content: `<@!${target.id}>, you have been unassigned from the ${role.name} role.` }
}