...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('dice').setDescription('Dice to roll (XdY)').setRequired(true))
        .addBooleanOption(option =>
            option.setName('verbose').setDescription('Set to true to return value of each rolled die')),
...
async function roll(dice) {
    if (!dice.match(/^[1-9]\d*d[1-9]\d*$/)) return { content: 'Please follow the syntax XdY, where X is number of dice and y is type of die.', ephemeral: true }
