...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option => option.setName('minutes').setDescription('Minutes to wait to collect votes').setRequired(true))
        .addStringOption(option => option.setName('question').setDescription('What are you voting on?').setRequired(true))
    ...
    execute: async (bot, message, args) => {
        ...
        const creator = (!message.member.nickname) ? message.author.username : message.member.nickname;

        return await createPoll(bot, channel, creator, args[0], question, optionsSliced);
    },
    result: async (bot, question, votes) => {
        const resultsEmbed = new MessageEmbed().setTitle('Poll results').setDescription(question).setColor('#004426');
    ...
    interact: async (interaction) => {
        const creator = (!interaction.member.nickname) ? interaction.SlashCommandBuilder.username : interaction.member.nickname
        ...
        const reply = await createPoll(interaction.client, channel, creator, minutes, question, options);
        interaction.reply(reply);
    },
    cast: async (messageId, option, user) => {
        const polls = JSON.parse(fs.readFileSync('./polls.json', 'utf-8'));
        if (!polls[messageId]) return { content: 'This poll has already ended.', ephemeral: true };
        for (v in polls[messageId]['votes']) { // check for duplicate votes
        ...
        fs.writeFile('./polls.json', JSON.stringify(polls, null, '\t'), err => { if (err) console.error(err) });
 ...
 */
async function createPoll(client, channel, creator, minutes, question, options) {
    const pollData = JSON.parse(fs.readFileSync('./polls.json', 'utf-8'));
    const date = new Date(Date.now() + (minutes * 60000));

    const selectMenu = new MessageSelectMenu().setCustomId('poll-options').setPlaceholder('Pick an option');
    const addOptions = [];
    ...
    fs.writeFile('./polls.json', JSON.stringify(pollData, null, '\t'), err => { if (err) console.error(err) });
