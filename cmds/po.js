...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("po")
    .setDescription(
      "Check on the status of a purchase through a PO number lookup",
    )
    .addIntegerOption((option) =>
      option
        .setName("po")
        .setDescription(
          "PO number to lookup (XXXXYY) where XXXX is current season (2122) and YY is assigned number",
        )
        .setRequired(true),
    ),
  ...
  execute: async (bot, message, args) => {
    if (!parseInt(args[0]))
      return "Please provide PO with format XXXXYY where XXXX is current season (2122 for 2021-2022) and YY is assigned number";
...
async function readSheet(po) {
  ...
  if (lookup === -1)
    return { content: "No order found with that PO number", ephemeral: true };
