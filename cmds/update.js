...
module.exports = {
    ...
    execute: async (bot, message, args) => {
        ...
        return await updateParameter(leadershipRole, message.member, args[0], args.slice(1).join(' '))
    ...
    interact: async interaction => {
        ...
        try {
            interaction.reply(await updateParameter(leadershipRole, interaction.member, parameter, value))
        } catch (error) {
            console.error(error)
...
async function updateParameter(leadershipRole, member, parameter, updatedValue) {
