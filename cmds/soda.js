...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('leaderboard').setDescription('Show the soda intake leaderboard'))
        .addSubcommand(subcommand =>
            subcommand.setName('drink').setDescription('Add sodas to the tracker')
            .addIntegerOption(option => option.setName('amount').setDescription('How many you drank. Defaults to 1 if not defined').setRequired(false)))
        .addSubcommand(subcommand => subcommand.setName('reset').setDescription('Resets the tracker')),
    ...
    interact: async interaction => {
        ...
        if (subCommand === 'leaderboard') interaction.reply(await showLeaderboard())
...
async function showLeaderboard() {
    ...
    const sortedSodas = sodas.slice(0).sort((a, b) => { return b.sodas - a.sodas })
...
async function drink(member, amount = 1) {
    ...
    if (objIndex === -1) {
        ...
        fs.writeFile('./soda.json', JSON.stringify(sodaJSON, null, '\t'), err => { if (err) console.error(err) })
        return reply
    }
    if (objIndex >= 0) {
        ...
        fs.writeFile('./soda.json', JSON.stringify(updatedSodas, null, '\t'), err => { if (err) console.error(err) })
        return reply
    }
}

async function reset() {
    fs.writeFile('./soda.json', JSON.stringify([]), err => { if (err) console.error(err) })
