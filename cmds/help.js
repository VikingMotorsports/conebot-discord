...
module.exports = {
    ...
    interact: async (interaction) => {
        ...
        if (command !== null) {
            interaction.reply(await showDetailedHelp(interaction.client.commands, command));
            return;
        }
...
async function showDetailedHelp(commands, commandHelp) {
    ...
    const detailedHelp = new Discord.MessageEmbed()
        .setColor('#004225');

    if (command.aliases) detailedHelp.addField('Aliases', command.aliases.join(', '));
    if (command.isSlashCommand) detailedHelp.setFooter('This command is also a slash command.');
    if (command.description) detailedHelp.setDescription(command.description);
    if (command.usage) detailedHelp.setTitle(`${prefix}${command.data.name} ${command.usage}`)
    else if (!command.usage) detailedHelp.setTitle(`${prefix}${command.data.name}`);
