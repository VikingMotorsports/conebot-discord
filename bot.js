...
bot.on("ready", async () => {
  console.log(
    `Bot online as ${bot.user.username} in ${bot.guilds.cache.first()}`,
  );
...
bot.on("messageCreate", async (message) => {
  ...
  if (
    message.content.toLowerCase().includes("sticky liquid" || "sticky juice")
  ) {
    message.react("💦");
  }
  if (
  ...
  if (
    message.content.toLowerCase().includes("can't believe you've done this") ||
    message.content.toLowerCase().includes("cant believe youve done this") ||
  ...
  if (commandName.startsWith(config.prefix)) {
    ...
    const command =
      ...
      bot.commands.find((cmd) => cmd.aliases && cmd.aliases.includes(cmds));
    try {
...
bot.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isSelectMenu() && interaction.customId === "poll-options")
